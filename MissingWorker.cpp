...
THREAD_PROC_RETURN_VALUE MissingWorkerThread(void* pParam)
{
	...
	sprintf(logmissingworkertaskfilename, LOG_FOLDER"logmissingworkertask_%.64s.csv", strtime_fns());
	LeaveCriticalSection(&strtimeCS);
	logmissingworkertaskfile = fopen(logmissingworkertaskfilename, "w");
	if (logmissingworkertaskfile == NULL)
	{
	...
	fprintf(logmissingworkertaskfile, 
		"%% Time (in s); Distance to the missingworker (in m);\n"
		); 
	fflush(logmissingworkertaskfile);
...
#pragma region Object detection
		...
		obji = 0;
		objj = 0;
		objMinRadius = (int)(videoimgwidth*objMinRadiusRatio_missingworker);
		objMinnbSelectedPixelsij = objMinRadius/2;

		// Get an image from the webcam or video.
		EnterCriticalSection(&imgsCS[videoid_missingworker]);
		cvCopy(imgs[videoid_missingworker], image, 0);
		LeaveCriticalSection(&imgsCS[videoid_missingworker]);
		...
		for (i = 0; i < image->height; i++)
		{
			for (j = 0; j < image->width; j++)
			{
				...
				if (
					//((r >= rmin_missingworker)&&(r <= rmax_missingworker))&&
					//((g >= gmin_missingworker)&&(g <= gmax_missingworker))&&
					//((b >= bmin_missingworker)&&(b <= bmax_missingworker))
					((h >= hmin_missingworker)&&(h <= hmax_missingworker))&&
					((s >= smin_missingworker)&&(s <= smax_missingworker))&&
					((l >= lmin_missingworker)&&(l <= lmax_missingworker))
					)
				{
...
#pragma endregion
		if (nbSelectedPixels == 0) 
		{
			...
			cvCopy(overlayimage, MissingWorkerOverlayImg, 0);
...
#pragma endregion
	...
	fclose(logmissingworkertaskfile);
