...
BlueROV2 support : Ensure that the parameter FRAME_CONFIG is set to vectored in ArduSub, assuming that the BlueROV2 Raspberry Pi is sending MAVLink data through UDP 192.168.2.2:14550, you will need to run mavproxy.exe --master=0.0.0.0:14550 --out=tcpin:0.0.0.0:5762 --out=tcpin:0.0.0.0:5763 to convert UDP to TCP (set client IP address and TCP port to 127.0.0.1:5762 in MAVLinkDevice0.txt, while Mission Planner/QGroundControl should also be able to connect through TCP 127.0.0.1:5763, but disable the joystick to avoid conflicts). Warning : ARMING_CHECK as well as other parameters might be modified by UxVCtrl.exe, make a backup of all ArduSub parameters before using it. Then, run UxVCtrl.exe from BLUEROV_WORKSPACE (ensure that robid is set to 0x00000040 and bDisableMAVLinkDevice[0] is set to 0 in UxVCtrl.txt, you might need to make other changes in the controller and observer parameters as well as in MAVLinkDevice0.txt depending on the dynamics of the robot). You might need to rearm manually the robot using . key on OpenCVGUI. The BlueROV2 video is not yet directly supported at the moment (third-party software would need to convert the video data from 192.168.2.2:5600 to a supported remote video stream format).
