...
THREAD_PROC_RETURN_VALUE PipelineThread(void* pParam)
{
	...
	sprintf(logpipelinetaskfilename, LOG_FOLDER"logpipelinetask_%.64s.csv", strtime_fns());
	LeaveCriticalSection(&strtimeCS);
	logpipelinetaskfile = fopen(logpipelinetaskfilename, "w");
	...
	fprintf(logpipelinetaskfile, 
		"%% Time (in s); Distance to the pipeline (in m);\n"
		); 
	fflush(logpipelinetaskfile);
	...
	for (;;)
	{
		...
		if (bExit) break;
		if ((!bPipelineDetection)&&(!bPipelineTrackingControl)) continue;
...
#pragma region Object detection
		...
		obji = 0;
		objj = 0;
		objMinRadius = (int)(videoimgwidth*objMinRadiusRatio_pipeline);
		objMinnbSelectedPixelsij = objMinRadius/2;

		// Get an image from the webcam or video.
		EnterCriticalSection(&imgsCS[videoid_pipeline]);
		cvCopy(imgs[videoid_pipeline], image, 0);
		LeaveCriticalSection(&imgsCS[videoid_pipeline]);
		...
		for (i = 0; i < image->height; i++)
		{
			for (j = 0; j < image->width; j++)
			{
				...
				if (
					//((r >= rmin_pipeline)&&(r <= rmax_pipeline))&&
					//((g >= gmin_pipeline)&&(g <= gmax_pipeline))&&
					//((b >= bmin_pipeline)&&(b <= bmax_pipeline))
					((h >= hmin_pipeline)&&(h <= hmax_pipeline))&&
					((s >= smin_pipeline)&&(s <= smax_pipeline))&&
					((l >= lmin_pipeline)&&(l <= lmax_pipeline))
					)
				{
...
#pragma endregion
		if (nbSelectedPixels == 0) 
		{
			...
			cvCopy(overlayimage, PipelineOverlayImg, 0);
...
#pragma region Object characteristics computations
			...
			objDistance = objRealRadius_pipeline/tan(objRadius*pixelAngleSize);
...
#pragma region Actions
			...
			fprintf(logpipelinetaskfile, "%f;%f;\n", 
				GetTimeElapsedChronoQuick(&chrono), objDistance
				);
			fflush(logpipelinetaskfile);

			if (bPipelineDetection)
			{
			...
			if (bPipelineTrackingControl)
			{
				...
				wtheta = Center(thetahat)-kh_pipeline*atan((objj-(double)videoimgwidth/2.0)/((double)videoimgwidth/2.0));
				//bDistanceControl = FALSE;
				//bBrakeControl = FALSE;
