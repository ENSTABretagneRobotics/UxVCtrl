...
THREAD_PROC_RETURN_VALUE SonarAltitudeEstimationThread(void* pParam)
{
	...
	for (;;)
	{
		...
		std::vector<double>::iterator i = std::min_element(Vdistances_filtered.begin(), Vdistances_filtered.end());
		if ((i != Vdistances_filtered.end())&&((*i) < rangescale)) altitude_sea_floor = (*i)*sin(0.5*40.0*M_PI/180.0);
